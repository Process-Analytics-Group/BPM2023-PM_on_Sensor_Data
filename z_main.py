...
def execute_process_model_discovery():
    ...
    raw_sensor_data_motion = helper.import_raw_sensor_data(filedir=settings.path_data_sources,
                                                           ...
                                                           sensor_labels=dict_distance_adjacency_sensor[
                                                               'sensor_labels'])
    ...
    # execute process model discovery with parameter search space (hyperopt parameter tuning)
        ...
        # add time of program start to filename
        filename_export_data = t0_datetime.strftime(settings.filename_export_data)
        # zip export files
        export_data = ZipFile(settings.path_data_sources + filename_export_data, 'w')
        # export data zip files should be ignored
        ed_pattern = re.compile(settings.filename_pattern_ed)
        # run throgh all contents in the folder
        for folderName, subfolders, filenames in os.walk(settings.path_data_sources + settings.dir_runtime_files):
            for filename in filenames:
                filePath = os.path.join(folderName, filename)
                if not ed_pattern.match(filename):
                    export_data.write(filePath, filePath)
    ...
    return perform_process_model_discovery.function_values
